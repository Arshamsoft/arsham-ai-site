...
const PageManager = () => {
  ...
  const pages = [
    ...
    { name: "اپلیکیشن‌های اندرویدی", path: "/android", status: "منتشر شده", lastModified: "۱ روز پیش" },
  ...
  const handleEdit = (page) => {
    // فرض می‌کنیم به صفحه ویرایش کارت‌ها می‌رود
    navigate(`/admin/edit/${page.path.replace("/", "") || "home"}`);
  };
  ...
  const handleDelete = (page) => {
    if (window.confirm(`آیا مطمئن هستید که می‌خواهید صفحه "${page.name}" را حذف کنید؟`)) {
      // حذف محتوای صفحه از localStorage
      localStorage.removeItem(`pageContent_${page.name}`);
      console.log("Deleted page:", page.name);
    }
  };
      ...
      {filteredPages.length === 0 && (
        <div className="bg-card rounded-lg border border-border p-12 text-center">
          ...
          <p className="text-muted-foreground mb-4">
            هیچ صفحه‌ای با این جستجو پیدا نشد.
          </p>
          <button
