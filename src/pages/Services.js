// frontend/src/pages/Services.js (جایگزین کن)
...
export default function Services() {
  ...
  useEffect(() => {
    api.get('/content').then((res) => {
      const list = res.data && res.data.services;
      if (Array.isArray(list) && list.length > 0) {
        setServices(list.map((item) => item.value || item));
      }
    }).catch(() => {});
  }, []);
    ...
    <div className="min-h-screen w-full bg-gray-50 dark:bg-gray-900 text-gray-900 dark:text-gray-100 p-8">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {services.map((item, i) => (
            <div key={i} className="bg-white dark:bg-gray-800 shadow-lg dark:shadow-gray-700 rounded-lg p-6 text-center transition hover:shadow-2xl hover:scale-105">
